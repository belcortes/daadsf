  ...
  $.getJSON("/resource", function(json) {
    $.each(json, function(key, data) {
      ...
      if (data.station_type == 'Police Station' && data.location == 'SF') {
        ...
        var contentInfo = ("Address:"+" "+data.street_number+" "+data.street_name+" "+data.street_type)
        ...
        var contentInfo = ("Name:"+" "+data.name+"</br>"+"Type:"+" "+data.station_type+"</br>"+"Address:"+" "+data.street_number+" "+data.street_name+" "+data.street_type)
...
function user_data() {
  ...
  $.getJSON("/users", function(json) {
    $.each(json, function(key, data) {
      ...
      var contentInfo = ("Name:"+" "+data.name+"</br>"+"Address:"+" "+data.address+"</br>"+"<div style='color:#0062A0; font-weight:bolder; cursor:pointer;'>Contact this owner</div>"+"</br>"+"Resources:"+"</br>"+trues)

      $('.contact-owner').on('click', function(){
        if ($('.user-contact').length == 0 ) {
          console.log('modal hasnt been created')
          var div = document.createElement('div');
        } else {
          console.log('already hurr')
          // $('.user-contact').remove()
        }

        
        // div.innerHTML ='<div class="modal-header">'
        // div.innerHTML ='  <button class="close" data-dismiss="modal">x</button>'
        
        // div.innerHTML ='</div>'
        // div.innerHTML ='<div class="modal-body">'
        // div.innerHTML =' <p>hello this is ownser bla</p> '
        // div.innerHTML ='</div>'
        ...
        if ($(this).prev().text().trim() == data.name) {
          ...
          div.innerHTML ='<div class="modal-header"> <button class="close" data-dismiss="modal">x</button> <h2>'+$(this).prev().text().trim()+'</h2> '
          // '</div>  <div class="modal-body"> '+'Email: '+data.email+'</br>Phone: '+data.phone+'</br>Address: '+data.address+data.address2+'San Francisco, CA'+'</div>'
...
function PrintContent() {
  var DocumentContainer = document.getElementById('user-print');
...
//   var icon = "/assets/cooling.png"
//   var icon2 = "/assets/person.png"
//   var bounds = new google.maps.LatLngBounds();
...
//           geocoder.geocode( { 'address': data.address+' '+data.address2+','+'San Francisco'+','+'CA'+','+data.zip}, function(results, status) {
//               console.log(data.address+' '+data.address2+','+'San Francisco'+','+'CA'+','+data.zip)
