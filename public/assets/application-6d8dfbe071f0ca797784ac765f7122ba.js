...
function boolean_quantities() {
  ...
  // $("#user-resources").find("input:not(checked)").next().hide()
  // $('#user-resources').find('input:not(checked)')
}
...
$(function() {
  ...
  $("#sign_up").bind("ajax:success", function(e, data, status, xhr) {
    if (data.success) {
      $('#account-container').show();
      // $('#sign_up').modal('hide');
      // $('#sign_up_button').hide();
