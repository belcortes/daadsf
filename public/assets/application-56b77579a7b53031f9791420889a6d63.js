...
function user_data() {
  ...
  $.getJSON("/users", function(json) {
    $.each(json, function(key, data) {
      ...
      var contentInfo = ("<strong>Name:</strong> "+" "+data.name+"</br>"+"<strong>Address:</strong> "+" "+data.address+"</br>"+"<div class='contact-owner' style='color:#0062A0; font-weight:bolder; cursor:pointer;'>Contact this owner</div>"+"<strong>Resources:</strong>"+"</br>"+trues)

      $('.contact-owner').on('click', function(){

          $('.close').on('click', function(){
            $('.user-contact').modal('destroy')
            // $('.user-contact').remove()
