    ...
    $.each(json, function(key, data) {
      ...
      $('.contact-owner').on('click', function(){
        ...
        if ($(this).prev().text().trim() == data.name) {
          ...
          div.innerHTML ='<div class="modal-header"> <button class="close" data-dismiss="modal">x</button> <h2>'+$(this).prev().text().trim()+'</h2> </div>  <div class="modal-body"> '+'Email: '+data.email+'</br>Phone: '+data.phone+'</br>Address: '+data.address+data.address2+'San Francisco, CA'+'</div>'
